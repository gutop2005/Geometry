...
while True:
	...
	py=pos[1]
	px=pos[0]
#to create a polygon Delete everything before # and the #	p.draw(win)
#	label(win,f'{p.Lpos[2-2]}',24,10,700)
#polygon colision#	if p.touch(pos,win):
#polygon colision event#		pygame.quit()
	#circle#c.draw(win)
#circle colision#	if c.touch1(pos, win):
	#event circle collision#	pygame.quit()
#triangle colision#	if t.touch(pos)==True:
#event triangle colision#		pygame.quit()
#triangle	t.draw(win)
